...
namespace TsMap.Canvas
{
    public partial class TsMapCanvas : Form
    {
        ...
        public TsMapCanvas(SetupForm f, string path, List<Mod> mods)
        {
            ...
            // BEZPIECZNE PRZYPISANIE PALETY (bez użycia operatora ??)
            if (_appSettings != null && _appSettings.MapColor != null)
            {
                _palette = _appSettings.MapColor.ToMapPalette();
            }
            else
            {
        ...
        private void SaveTileImage(int z, int x, int y, PointF pos, float zoom, string exportPath, RenderFlags renderFlags)
        {
        ...
        private void GenerateTileMap(int startZoomLevel, int endZoomLevel, string exportPath, bool createTiles, bool saveInfo, RenderFlags renderFlags)
        {
            ...
            Task.Run(() =>
            {
                ...
                {
                    ZoomOutAndCenterMap(tileSize, tileSize, out PointF pos, out float zoom);
                    if (saveInfo)
                    {
        ...
        {
            if (_paletteEditorForm == null || _paletteEditorForm.IsDisposed)
            {
                _paletteEditorForm = new PaletteEditorForm(_appSettings, () =>
                {
                    // BEZPIECZNE PRZYPISANIE PALETY (bez użycia operatora ??)
                    if (_appSettings != null && _appSettings.MapColor != null)
                    {
        ...
        private void GenerateTileMapToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            _tileMapGeneratorForm.GenerateTileMap += (exportPath, startZoomLevel, endZoomLevel, createTiles, exportFlags, renderFlags) =>
            {
                _tileMapGeneratorForm.Close();
                _appSettings.LastTileMapPath = exportPath;
                JsonHelper.SaveSettings(_appSettings);
